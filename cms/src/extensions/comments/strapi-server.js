...
const resolveRelationEntryId = async (identifier) => {
  if (!identifier) {
    return null;
  }
  ...
  if (NUMERIC_PATTERN.test(identifier)) {
    ...
    if (Number.isFinite(numeric) && numeric > 0) {
      const postById = await fetchPostByWhere({ id: numeric });
      entryId = coerceEntryId(postById?.id ?? postById?.entryId ?? postById?.entry_id ?? numeric);
    }
  }
  ...
  if (!entryId) {
    ...
    entryId = coerceEntryId(byDocumentId?.id ?? byDocumentId?.entryId ?? byDocumentId?.entry_id);
  }

  if (!entryId) {
    const bySlug = await fetchPostByWhere({ slug: identifier });
    entryId = coerceEntryId(bySlug?.id ?? bySlug?.entryId ?? bySlug?.entry_id);
  }

  cacheRelationIdentifier(identifier, entryId);
  return entryId;
};
...
const normalizeRelation = async (relation) => {
  ...
  const relationEntryId = await resolveRelationEntryId(identifier);
  if (!relationEntryId) {
    return relation;
  }

  return `${RELATION_PREFIX}${relationEntryId}`;
};
...
const normalizeStoredCommentRelations = async (strapiInstance) => {
  ...
  while (true) {
    ...
    for (const comment of comments) {
      ...
      const normalized = await resolveRelationEntryId(identifier);
      if (!normalized) {
        continue;
      }
  ...
  if (updated > 0) {
    strapiInstance.log.info(
      `[comments] Normalized ${updated} stored comment relations to entry IDs (inspected ${inspected}).`,
    );
  }
};
...
`,
  ...
  plugin.bootstrap = async (...args) => {
    ...
    await normalizeStoredCommentRelations(bootstrapStrapi || strapi);
  };
