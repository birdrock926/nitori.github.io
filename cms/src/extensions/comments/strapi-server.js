...
const withSanitizedLimit = (controller) => {
  ...
  return async function controllerWithSanitizedLimit(ctx, next) {
    if (ctx) {
      ...
      const limit = sanitizeCommentsLimit(ctx.query);
...
const resolveRelationIdentifier = async (identifier) => {
  if (!identifier) {
    return null;
  }
  ...
  relationCache.set(identifier, relationIdentifier);
...
const normalizeRelation = async (relation) => {
  ...
  const relationIdentifier = await resolveRelationIdentifier(identifier);
  if (!relationIdentifier) {
    return relation;
  }

  return `${RELATION_PREFIX}${relationIdentifier}`;
};
