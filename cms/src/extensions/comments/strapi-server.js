...
const withSanitizedLimit = (controller) => {
  ...
  return async function controllerWithSanitizedLimit(ctx, next) {
    ...
    if (ctx) {
      ...
      appliedLimit = sanitizeCommentsLimit(ctx.query);
    ...
    if (ctx?.body) {
      normalizePaginationMeta(ctx.body, appliedLimit ?? ctx?.state?.commentsLimit);
    }
...
const resolveRelationDocumentId = async (identifier) => {
  if (!identifier) {
    return null;
  }
  ...
  if (!relationDocumentId) {
    ...
    if (direct) {
      relationDocumentId = coerceDocumentId(direct);
    }
  }

  if (!relationDocumentId) {
    const bySlug = await fetchPostByWhere({ slug: identifier });
    relationDocumentId = coerceDocumentId(bySlug);
  }

  relationCache.set(identifier, relationDocumentId);
  return relationDocumentId;
};
