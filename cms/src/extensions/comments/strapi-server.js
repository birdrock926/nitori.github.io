...
const resolveRelationDocumentId = async (identifier) => {
  ...
  if (NUMERIC_PATTERN.test(identifier)) {
    const postById = await fetchPostByWhere({ id: Number(identifier) });
    documentId = coerceDocumentId(postById);
  }
  ...
  if (!documentId) {
    ...
    documentId = coerceDocumentId(byDocumentId) || documentId;
  }
...
const normalizeStoredCommentRelations = async (strapiInstance) => {
  ...
  while (true) {
    ...
    for (const comment of comments) {
      ...
      const normalized = await resolveRelationDocumentId(identifier);
