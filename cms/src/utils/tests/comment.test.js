...
describe('コメントユーティリティ', () => {
  ...
  test('健全な本文はモデレーション不要', () => {
    const result = evaluateModeration('今日はとても楽しいニュースです。');
  ...
  test('英語の暴言もモデレーション対象になる', () => {
    const result = evaluateModeration('This comment is full of fuck words');
    expect(result.requiresReview).toBe(true);
    expect(result.reasons.some((reason) => reason.type === 'word')).toBe(true);
    expect(result.severity).toBe('high');
  });
  ...
  test('レート制限', async () => {
    ...
    await expect(
      enforceRateLimit(strapi, { ipHash: 'abc', overrides: { minute: 1, hour: 5, day: 10 } })
  ...
  test('paginateComments は公開済みコメントのみを返す', async () => {
    ...
    expect(mockCount).toHaveBeenCalledWith('api::comment.comment', {
      filters: expect.objectContaining({ status: { $eqi: 'published' } }),
    });
    ...
    expect(pagination).toEqual({ page: 1, pageSize: 10, pageCount: 1, total: 1 });
  });
});
