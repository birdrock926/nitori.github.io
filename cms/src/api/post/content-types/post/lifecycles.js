...
const normalizeMarkdownWhitespace = (value) =>
  value
    .replace(/\r\n?/g, '\n')
    .replace(/<br\s*\/?>(?=\s|$)/gi, '\n')
    .replace(/&nbsp;/gi, ' ');
...
const normalizeRichTextBlock = (block) => {
  ...
  const alignmentAliases = ['alignment', 'textAlign', 'text_alignment', 'align'];
  let alignmentValue = null;

  for (const key of alignmentAliases) {
    if (typeof next[key] === 'string' && !alignmentValue) {
      const candidate = next[key].trim().toLowerCase();
      if (candidate) {
        alignmentValue = candidate;
      }
    }

    if (key !== 'alignment' && Object.prototype.hasOwnProperty.call(next, key)) {
      delete next[key];
    }
  }

  if (alignmentValue && RICH_TEXT_ALIGNMENTS.has(alignmentValue)) {
    next.alignment = alignmentValue;
  } else {
