...
import {
  AUTO_PUBLISH_MODES,
  normalizeAutoPublishSetting,
} from '../../../modules/comments/constants.js';
import { validateBody, evaluateModeration } from '../../../modules/comments/moderation.js';
import { hashIp, networkHash, createEditKey, hashEditKey } from '../../../modules/comments/crypto.js';
import { resolveAlias } from '../../../modules/comments/alias.js';
import {
  isBanned,
  enforceRateLimit,
  detectSimilarity,
} from '../../../modules/comments/rate-limit.js';
import {
  paginateComments,
  toPublicComment,
} from '../../../modules/comments/transform.js';
import { createClientMeta, readClientMeta, mergeModerationMeta } from '../../../modules/comments/meta.js';
import { findPublishedPostBySlug } from '../../../modules/comments/slug.js';
...
export default factories.createCoreController('api::comment.comment', ({ strapi }) => {
  ...
  async submit(ctx) {
    ...
    const { sanitized: sanitizedBody, requiresReview, reasons, score, severity } = evaluateModeration(body);
    ...
    const publishSetting = normalizeAutoPublishSetting(process.env.COMMENTS_AUTO_PUBLISH);
    const forceManualReview = publishSetting === AUTO_PUBLISH_MODES.MANUAL;
    const shouldPublishNow =
      publishSetting === AUTO_PUBLISH_MODES.ALWAYS ||
      (publishSetting === AUTO_PUBLISH_MODES.SMART && !requiresReview);
    ...
    const record = await strapi.entityService.create('api::comment.comment', {
      data: {
        ...
        status: shouldPublishNow ? 'published' : 'pending',
        meta: {
          client: createClientMeta({ ip, ua, submittedAt: submittedAtIso }),
          display: {
            aliasProvided: aliasData.provided,
            postTitle: postEntry.title || null,
            postSlug: postEntry.slug || null,
          },
          moderation: {
            requiresReview: requiresReview || forceManualReview,
            reasons,
            score,
            severity,
            reportCount: 0,
    ...
    publicComment.status = shouldPublishNow ? 'published' : 'pending';
  ...
  async list(ctx) {
    ...
    const { postSlug, page = '1', pageSize = '20' } = ctx.query || {};
    if (!postSlug) {
      return ctx.badRequest('記事が指定されていません');
    }
    ...
    const numericPageSize = Math.min(Math.max(parseInt(pageSize, 10) || 20, 5), 50);
    const numericPage = Math.max(parseInt(page, 10) || 1, 1);
    const { data, pagination } = await paginateComments(strapi, {
      postId,
      page: numericPage,
    ...
    return {
      data: {
        ...
        moderation: {
          ...
          score: typeof moderationMeta.score === 'number' ? moderationMeta.score : 0,
          severity: moderationMeta.severity || 'none',
        },
