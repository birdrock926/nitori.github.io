...
const normalizePublishSetting = (value) => {
  if (value === undefined || value === null) {
    return 'smart';
  }
  const normalized = value.toString().trim().toLowerCase();
  if (['manual', 'moderated', 'false', 'off', 'pending'].includes(normalized)) {
    return 'manual';
  }
...
export default factories.createCoreController('api::comment.comment', ({ strapi }) => {
  ...
  async submit(ctx) {
    ...
    const publishSetting = normalizePublishSetting(process.env.COMMENTS_AUTO_PUBLISH);
    const forceManualReview = publishSetting === 'manual';
    const shouldPublishNow =
      publishSetting === 'always' || (publishSetting === 'smart' && !requiresReview);
