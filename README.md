...
- **コメント**は Strapi Comments API（VirtusLab プラグイン）を REST で取得・投稿し、記事の **Document ID** を正規キーとして保存（数値エントリー ID や slug が送信されてもバックエンドが該当 Document ID へ再解決）。React 島がフォームとツリー UI を提供。
- **CMS（/cms）**：Strapi v5
