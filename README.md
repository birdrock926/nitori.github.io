...
## 成果物（Deliverables）
- **公開サイト**：Astro + React Islands を Node アダプター（`@astrojs/node`）で配信する **ハイブリッド SSR サイト**（一覧・LP はビルド時に生成しつつ、`/posts/[slug]` は SSR で Strapi の最新スラッグを即時反映）
...
## アーキテクチャ（High Level）
- **フロント（/web）**：Astro + React Islands（`output: 'hybrid'`）
