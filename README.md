...
- **コメント**は Strapi Comments API（VirtusLab プラグイン）を REST で取得・投稿し、記事の **Document ID** をスレッドキーとして利用（過去にエントリー ID や slug で保存されていたレコードも、起動時に Document ID へ自動補正）。React 島がフォームとツリー UI を提供。
- **CMS（/cms）**：Strapi v5
...
### SEO / 収益
- `WebSite` + `Organization` + `SearchAction` JSON-LD を常時配信し、投稿ページでは `NewsArticle` スキーマに発行日時・更新日時・タグ由来のキーワードを含めた詳細構造化データを出力
