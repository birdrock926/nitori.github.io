...
## アーキテクチャ（High Level）
- **フロント（/web）**：Astro Hybrid（本番は静的書き出し、開発時は SSR フォールバック）
  - 本番ビルドでは **Strapi の published のみ**を取得して静的化。開発サーバでは SSR がオンデマンドで新規スラッグを解決するため、CMS で記事を公開しても `npm run dev` を再起動する必要がありません。
