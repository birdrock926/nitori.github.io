...
- Rich Text ブロックの本文は CKEditor/Markdown の装飾（太字・斜体・取り消し線・インライン/ブロックコード・引用・リンク・画像・改行）を `marked` で HTML に変換します。Strapi 側からプレーンテキストが返ってきても、公開ページでは装飾と改行が正しく再現されます。レンダリング時にも同じ正規化を再実行するため、記事 API を別の処理で変換し損ねた場合でも公開画面で Markdown がそのまま表示されることはありません。
