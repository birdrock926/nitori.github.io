...
- **コメント**は Strapi Comments API（VirtusLab プラグイン）を REST で取得・投稿し、記事の **数値エントリー ID** をスレッドキーとして利用（Document ID や slug が渡された既存レコードも起動時に自動でエントリー ID へ補正）。React 島がフォームとツリー UI を提供。
- **CMS（/cms）**：Strapi v5
...
- **コメント**：Strapi プラグイン（strapi-plugin-comments）が `plugin::comments.comment` として保存し、記事 (`api::post.post`) のエントリー ID（Document ID や slug からの自動フォールバック付き）と紐付け
...
4. コメント API のベース URL は `https://<CMS>/api/comments/api::post.post:<entryId>` です。Astro 側は記事のエントリー ID をスレッドキーとして利用し、Document ID や slug が渡された場合でもバックエンドが自動的にエントリー ID へ補正して処理します。
