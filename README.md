...
- **ページネーション**：1 ページ 20 件のツリー表示と「前へ/次へ」ナビゲーションで大量コメントも軽快に閲覧
...
- Strapi 起動時に `cms/src/utils/bootstrap.js` が Content Manager の設定を強制適用し、Windows 環境でもコロン (`:`) を含むパスを作成せずに **対象記事 / ステータス / 投稿日時** の 3 列が必ず表示されるようにしました。既存のレイアウトが壊れていても再起動時に自動で修復されます。
- Content Manager の「Comments」一覧は **対象記事 / ステータス / 投稿日時** の 3 列を固定表示し、バッジ色で状態を識別できるようにしました。どの記事向けの投稿かを一覧から即座に判断できます。
1. **審査・公開**
   - Strapi 管理画面の「コンテンツマネージャー → Comments」で `pending` 状態の投稿を確認します。カスタムレイアウトにより「対象記事 / 投稿者 / 状態 / 投稿日時」が常に表示され、`状態` 列はバッジ表示で一目で判別できます。
   - 該当コメントを開き、右側の `status` プルダウンから `published` を選択し、画面右上の **保存** をクリックします。更新後は即座に Web 側へ反映されます。
   - Web フロントのコメント欄は 20 件単位でページングされるため、公開直後のコメントは 1 ページ目に集約され、モデレーターもすぐ確認できます。
...
4. **管理画面での文脈確認**
   - コメント編集画面では左上に対象記事、右カラムに現在のステータスが常に表示されます。詳細画面の JSON ビューでは `meta.display.postTitle` / `postSlug` に加え、親コメント (`parent`) の ID が確認できるため、返信ツリーの関係性も把握できます。
5. **自動判定と通報**
