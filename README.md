...
## 成果物（Deliverables）
- **公開サイト**：Astro + React Islands で生成する **静的サイト**（Strapi Webhook で再ビルドし、公開記事のスラッグを `getStaticPaths` で取得）
...
## アーキテクチャ（High Level）
- **フロント（/web）**：Astro 静的サイト生成（React Islands）
  - `getStaticPaths()` で Strapi の公開記事スラッグを収集し、`astro build` 時に `/posts/[slug]` を含むページをすべて静的生成します。Strapi で記事を公開すると Webhook → GitHub Actions → Cloudflare Pages の再ビルドが走り、数分以内に新しい記事が反映されます。ローカル開発中に記事を追加した場合は、Astro 開発サーバーを再起動して最新スラッグを取得してください。
...
3. **配信**：Astro が API（published のみ）を取得 → 静的 HTML/Assets を生成 → Cloudflare Pages（または任意の静的ホスティング）で公開
...
本リポジトリは Strapi v5 を用いた CMS(`/cms`) と Astro + React Islands を用いたフロントエンド(`/web`) のモノレポです。OCI Always Free 上で稼働する Docker Compose 環境を標準とし、フロントエンドは Cloudflare Pages 等へ静的デプロイします。Strapi 側は OCI が付与するホスト名（例: `https://xxx.adb.ap-tokyo-1.oraclecloudapps.com`）をそのまま利用し、独自ドメインを経由しない前提で各種 URL を設定してください。旧構成として Node アダプター経由の SSR 手順も履歴には残っていますが、現在は静的ビルド + CDN 配信が推奨です。
...
> ⚠️ 現在の標準構成は **静的ビルド + Cloudflare Pages** です。以下に旧来の SSR 手順も記載していますが、動的ルートは `getStaticPaths` で網羅しているため追加の Node サーバーは不要です。OCI 上の Strapi へは付与されたホスト名で直接接続してください。
