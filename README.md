...
3. 代表的なキーの説明:
  ...
  | `COMMENTS_AUTO_PUBLISH` | `smart`（既定）で禁止語・リンク違反のみ審査待ち、`always` で全件即時公開、`manual` で常にモデレーション待ち。 | `smart` |
...
> **Tips**
> - CAPTCHA を本番で有効化する場合は `COMMENTS_AUTO_PUBLISH=manual` に設定すると、すべての投稿がモデレーション待ちになります。フラグに触れない通常コメントも即時公開したい場合は既定の `smart` のままで構いません。
...
4. **管理画面での文脈確認**
   - Content Manager のコメント一覧に「対象記事」列を追加しているため、どの記事への投稿かをワンクリックで把握できます。詳細画面の JSON 表示からも `meta.client` 情報を確認できます。
5. **自動判定と通報**
   - 本文には禁止語・未許可ドメイン・過剰なリンク数のインデックスが適用され、該当した場合は `pending` として保存し、理由を `meta.moderation.reasons` に記録します。`COMMENTS_AUTO_PUBLISH=smart` の既定値では、これらに触れない投稿は自動的に `published` となります。
