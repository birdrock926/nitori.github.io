...
- **コメント**は Strapi Comments API（VirtusLab プラグイン）を REST で取得・投稿し、記事の **Document ID** をスレッドキーとして正規化（数値エントリー ID や slug が送信された場合でもバックエンドが Document ID へ再解決）。React 島がフォームとツリー UI を提供。
- **CMS（/cms）**：Strapi v5
...
- **REST API**：`/api/comments/api::post.post:<documentId>` を Astro 側が呼び出し、React 島が UI と投稿フォームを提供します（数値 ID や slug しか取得できない環境でもバックエンドが Document ID へ正規化してから保存するため、既存レコードとの整合性が保たれます）。
...
- **コメント**：Strapi プラグイン（strapi-plugin-comments）が `plugin::comments.comment` として保存し、記事 (`api::post.post`) の Document ID（数値 ID からの自動フォールバック付き）と紐付け
...
4. コメント API のベース URL は `https://<CMS>/api/comments/api::post.post:<documentId>` が正規形です。Astro 側は Document ID を最優先で送信し、数値エントリー ID や slug しか取得できない場合でもバックエンドが Document ID へ再解決してから保存します。
...
- コメントは Strapi 管理画面の **Comments → All** で確認できます。Document ID や slug で投稿されていた古いコメントも、起動時の正規化処理によって Document ID に統一されるため、リロードすれば該当記事のスレッドが表示されます。
