...
6. 返信コメントが付くと、元コメントの著者メールアドレス宛に Strapi のメールプラグインから通知が送信されます。VirtusLab Comments 3.1.0 では投稿時にメールアドレスが必須のため、フォームが空欄／不正な場合はバックエンドが自動で `@comments.local` ドメインのハッシュ済みダミーアドレスを補完して送信を受け付けます（このダミー宛には通知しません）。実際に返信通知を受けたい場合は有効なメールアドレスを入力し、SMTP（`SMTP_*`）と `COMMENTS_CONTACT_EMAIL` を設定してからテスト送信で動作確認してください。
