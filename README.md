...
- Rich Text ブロックの本文は CKEditor 由来の Markdown 互換テキストを `marked` ベースのレンダラーで HTML に変換します。太字/斜体/取り消し線/インライン・ブロックコード/引用/リンク/画像/リスト/改行をサポートし、Strapi からプレーンテキストが返ってきても公開ページでは装飾を保持します。描画時にも同じ正規化を再実行し、`<p>` や `<br>` だけでラップされた Markdown 断片は一旦剥がしてから再評価するため、`**bold** _italic_` などが素のまま表示されることはありません。
