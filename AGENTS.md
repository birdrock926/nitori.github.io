...
- **2025-11-03 追補**: CKEditor が `<p>…</p>` ラップを含む Markdown 風テキストを返すケースが残っていたため、`normalizeRichMarkup()` に段落/改行タグを剥がしてから Markdown として再評価するロジックを追加。`<p>`/`<br>` 以外の HTML タグが含まれている場合は既存マークアップを優先するようフィルターしている。【F:web/src/lib/strapi.ts†L176-L214】
