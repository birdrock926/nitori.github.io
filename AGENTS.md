...
### 2025-10-29 追記: b711719576ff6f43b94b728312c60ab912e52743 へのリストア

- **背景**: コメント管理画面が空になる不具合が継続していたため、安定稼働が確認できていたコミット `b711719576ff6f43b94b728312c60ab912e52743` の状態へコード一式を戻すよう指示を受けた。
- **対応**: `git read-tree --reset -u b711719576ff6f43b94b728312c60ab912e52743` 相当の処理で作業ツリーを該当コミットの内容に揃え、追加改変が無いことを確認した。
- **検証**: 実行していない（依存関係が未導入のため）。必要に応じて `cd cms && npm install` / `npm run develop` などを行って再度挙動を確認する。
- **メモ**: 今後差分を加える場合は、この状態から派生させつつコメント拡張やフロントエンドのロジックを丁寧に検証すること。

### 2025-10-30 追記: Comments 管理画面の Document ID 正規化復元

- **背景**: 依然として Strapi 管理画面の Comments 一覧が空のままで、サーバーログに `Relation for field "related" does not exist.` が出力される事象が継続。以前のタスクでコメントの `related` を数値 ID へ正規化していたが、VirtusLab Comments 3.1 系は Document ID (`documentId`) を前提に関連エンティティを解決するため、管理画面でレスポンス生成に失敗していた。併せて、limit 正規化フックが `ctx.request.querystring` を更新していなかったため、Knex が不正な limit を受け取り続けるケースも判明した。
- **対応**:
  1. `cms/src/extensions/comments/strapi-server.js` の正規化ロジックを全面的に見直し、コメント関連エンティティの解決結果として Document ID を優先的に返すよう修正。数値 ID と slug は最後のフォールバック扱いにし、結果を `relationCache` に保存して再解決を抑制。【F:cms/src/extensions/comments/strapi-server.js†L1-L255】
  2. 同ファイルの `withSanitizedLimit()` で `sanitizeCommentsLimit()` の返り値を用いて `ctx.request.querystring` / `ctx.querystring` / `ctx.state.query` を同期し、Strapi 本体へ渡るクエリが常に 1〜200 の整数になるよう補強した。【F:cms/src/extensions/comments/strapi-server.js†L46-L129】
  3. `cms/src/index.js` のブートストラップ処理も Document ID ベースへ揃え、既存コメントの `related` を `api::post.post:<documentId>` へ書き換えるよう更新。キャッシュとフォールバックの優先順位を管理画面側と統一した。【F:cms/src/index.js†L9-L116】
  4. フロントエンドのコメント UI（`web/src/components/comments/CommentsApp.tsx`）で関係候補の優先順位を Document ID → 数値 ID の順へ変更し、Strapi 側の期待と一致させた。【F:web/src/components/comments/CommentsApp.tsx†L180-L209】
- **検証**: 依存が未導入のサンドボックスのためコマンド実行は行っていない。ローカル検証時は `cd cms && npm install` → `npm run develop` で管理画面の Comments ページを開き、リレーション警告が解消されたことを確認すること。フロントエンドは `cd web && npm install` → `npm run dev` でコメント投稿・一覧表示を動作確認する。
- **備考**: 今後コメントの関連フィールドを操作する際は Document ID を第一候補とする。既存データに数値 ID が混在している場合も今回の正規化で Document ID へ揃うため、外部ツールでコメントを投入する場合は `api::post.post:<documentId>` を指定する運用に戻す。limit 周りの挙動が再度不安定になった場合は、`withSanitizedLimit()` が `ctx.request.querystring` を正しく書き戻しているか確認し、追加のエイリアスが必要なら `aliasKeys` へ加える。
