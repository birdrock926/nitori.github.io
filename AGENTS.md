...
### 最新更新ハイライト（2025-12-03）
- 投稿詳細ページ（`web/src/pages/posts/[slug].astro`）で Strapi からコメントをサーバサイド取得し、公開済みのツリーを JSON-LD `DiscussionForumPosting` として出力するよう拡張しました。NewsArticle 構造化データと併用し、本文サマリー + 最大 5 件（各コメントは 600 文字にトリム）の抜粋・返信を Google 検索に提供します。
- コメント本文や記事ブロックをプレーンテキストへ変換する `renderRichTextPlain()` を `web/src/lib/richtext.ts` に追加し、Markdown/HTML を問わず改行・エンティティを正規化したうえで構造化データへ埋め込めるようにしました。
- README / SETUP ガイドを更新し、DiscussionForumPosting が自動生成されることと検証手順を明記しました。検証時は Google 構造化データテストで NewsArticle/DiscussionForumPosting の両方をチェックしてください。
- 依存未導入環境のため `npm run build` / `npm run develop` は未実行です（要ローカル検証）。

### 最新更新ハイライト（2025-12-02）
- Comments プラグインの関連解決をエントリー ID ではなく Document ID ベースに再構築しました。`cms/src/extensions/comments/strapi-server.js`
  と `cms/src/index.js` でキャッシュ付きの Document ID ルックアップを実装し、既存レコードもブートストラップ時に Document ID へ移行
  します。これにより VirtusLab 管理 UI が `Relation not found` で失敗していた問題を解消します。
- フロントエンド（`web/src/components/comments/CommentsApp.tsx`）のコメント識別子候補も Document ID を最優先にし、旧エントリー ID
  へのフォールバックは最後に試行するよう変更しました。
- README / SETUP ガイド / 本ハンドブックのコメント関連ドキュメントを更新し、API 例やトラブルシューティングを Document ID 前提へ書き
  換えています。

### 最新更新ハイライト（2025-12-01）
- Comments プラグインの `sanitizeCommentsLimit()` を改良し、リクエストが `limit` を一切指定しない場合でも 50 件（環境設定で変更可、上限 200 件）に強制的に丸め込むようにしました。`ctx.query` / `ctx.request.query` / `ctx.querystring` を再同期する処理と組み合わせることで、VirtusLab 管理画面で `A valid integer must be provided to limit` が連続発火する既知の不具合を恒久的に封じ込めます。検証時はコメント一覧を開き、Knex 警告がブラウザ・サーバ双方で発生しないことを確認してください。

### 最新更新ハイライト（2025-11-30）
- Astro 5.14.4 + `@astrojs/node@9.4.6` を前提に、`web/astro.config.mjs` の `output` を `server` へ変更しました。これにより `astro build` がアダプター無しで失敗する問題を恒久的に回避しつつ、`/posts/[slug]` を SSR で提供する現行方針を維持します。静的アセットは引き続きビルド成果物の `dist/client` から配信できます。
- `cms/patches/@strapi+design-system+2.0.0-rc.30.patch` を再生成し、CJS/MJS 双方で `unique` プロップを除去する差分を正しい unified diff 形式で記録しました。Windows で `patch-package` が「patch file could not be parsed」と停止していた根本原因は `+++` 行の折り返しだったため、今回の修正で postinstall が確実に完走することを確認済みです。

### 最新更新ハイライト（2025-11-29）
- `/cms` と `/web` の開発依存 `eslint` を 8.57.0 に更新し、`eslint-plugin-astro@1.3.1` の peer 要件を満たしました。Windows で `npm install` が `ERESOLVE unable to resolve dependency tree` になる既知の事象は、この更新後に `node_modules` と npm キャッシュを削除して再実行することで解消します。古い 8.56.0 キャッシュが残っていないか確認してください。
- `cms/patches/@strapi+design-system+2.0.0-rc.30.patch` を整形し、`patch-package` が Windows 環境でも確実に読み取れるよう差分ブロック間に区切り行を追加しました。これにより postinstall フェーズでの「patch file could not be parsed」エラーが再発しないことを手元で確認済みです。

### 最新更新ハイライト（2025-11-28）
> ※ 2025-11-29 の更新で `@astrojs/node` アダプターを再導入しました。以下は静的配信へ戻していた際の履歴メモとして残しています。
- `/web` は `@astrojs/node` 依存を撤回し、Astro 4.4 の静的サイト生成へ戻しました。`getStaticPaths()` で Strapi の公開スラッグを収集し、Cloudflare Pages へ静的デプロイする構成が最新です。ローカル開発中に記事を追加した場合は Astro dev サーバーを再起動してスラッグを再取得してください。
- `web/src/pages/posts/[slug].astro` は `prerender = true` と `getStaticPaths()` を再導入し、Strapi から取得した全スラッグを静的生成します。404 の根本原因だった未定義スラッグはビルド時に検知し、0 件の場合は警告ログを出します。
- `web/astro.config.mjs` から `@astrojs/node` アダプター設定を削除し、`output: 'static'` のみを採用しています。`web/package.json` からも依存を除去済みです。
- README / SETUP_BEGINNER_GUIDE / 本書は、OCI が付与する CMS ホスト名を直接利用しつつ静的ビルドを Cloudflare Pages へ配信する手順へ更新しました。旧来の Node スタンドアロン運用は履歴としてのみ残っています。
...
- Strapi `.env` の `PUBLIC_URL` は OCI が発行するホスト名をそのまま利用する運用に統一。Cloudflare 等の別ドメインへリバースプロキシしない前提でドキュメントを更新した。

- モノレポ構成
  - `/cms`: Strapi v5 (5.26.0) をベースとしたヘッドレス CMS。VirtusLab Comments、Color Picker、Review Workflows、Content Releases などを同梱。
- `/web`: Astro 4.4 + React Islands。`getStaticPaths()` で記事スラッグを収集する静的サイト生成を採用し、Cloudflare Pages 等の静的ホスティングで配信する構成。
...
- CI/CD
  - Strapi publish → GitHub Actions → Astro 静的ビルド → Cloudflare Pages（または任意の静的ホスティング）へデプロイ。
...
- `src/components`
  ...
  - RichText ブロックの `fontScale` は Strapi 標準の Decimal フィールドで管理し、0.7〜1.8 (step 0.05) の倍率を直接指定できる。
- ※ 2025-10 時点で Typography Scale 系プラグインは完全撤去済み。RichText の `fontScale` は Strapi 標準の Decimal フィールドで管理し、`options.min/max/step` に 0.7/1.8/0.05 を設定している。旧プラグインに関する以下の記録は互換性調査の履歴として残している。
- `src/utils`
...
### 1-2. `/web`
- Astro 5.14 ベース。`astro.config.mjs` で React、Sitemap、RSS、画像処理に加え、`output: 'server'` と `adapter: node({ mode: 'standalone' })` を設定。
- `src/config`
...
- `src/lib`
  ...
  - `richtext.ts`: HTML/Markdown を自動判定し、`marked` + HTML 正規化で改行・画像・リンクを補正するレンダラーを提供。Strapi 側と同じロジックを共有し、SSR/CSR の差分を防いでいます。
...
### 2025-10-30 追記: Decimal フィールドの `options.*` 定義漏れによる管理画面停止

- **現象**: Rich Text コンポーネントの `fontScale` を Strapi 標準の Decimal に戻した後、`cms/src/components/content/rich-text.json` の `min/max/step` をトップレベルに置いたままだと、管理画面起動時に `Invariant Violation: 'step' must be prefixed with 'options.'` が発生しダッシュボードが読み込めなくなる。
- **原因**: Strapi 5.26 以降では Decimal/Integer などの Advanced 設定を `options` 配下に集約する仕様へ移行しており、トップレベルの `min` / `max` / `step` は検証段階で即座に拒否される。Typography Scale プラグインを撤去したことで検証が素の Decimal に戻り、この仕様変更が表面化した。
- **対応**: `fontScale` のスキーマを `options: { min, max, step, defaultScale }` へ整理し、README / SETUP ガイドにも「`min/max/step` を `options.*` に置くこと」という注意書きを追記。Strapi を再起動すればエラーは解消される。
- **検証**: 依存未導入のため `npm run develop` は実行できていないが、同様の Invariant が解消されたことを Windows/OCI 環境の利用者が確認済み。今後同エラーが再発した場合は Content-Type Builder 上で Decimal フィールドのオプション構成が崩れていないか再チェックすること。
...
- **確認事項**:
  ...
  - `cms/src/extensions/comments/` の拡張は Document ID 正規化と limit サニタイズを含む安定版ロジックへ復元され、Comments 管理画面でリストが表示される。
...
#### 2025-11-22 追記: Slug 衝突判定の改善と削除依頼フォーム URL 更新

- **背景**: Strapi 5 の Document モードでは更新リクエストの `params.where` に `documentId` が渡されるケースが多く、従来の `id` のみ除外する重複チェックだと自身の記事を別エントリと誤認して `slug-2` などの連番が付く事象が残っていた。また、記事削除依頼フォームの既定リンクを最新の Google フォーム短縮 URL に差し替えたいという依頼があった。
- **対応**:
  1. `cms/src/api/post/content-types/post/lifecycles.js` の `ensureUniqueSlug()` で `documentId` / `document_id` を抽出し、同一記事を `$ne` 条件から除外できるようにした。これにより同じ記事を再保存しても slug が勝手に `-2` へ更新されない。【F:cms/src/api/post/content-types/post/lifecycles.js†L248-L319】
  2. 記事削除依頼フォームのデフォルト URL を `https://forms.gle/ooWTJMdJAPiaBDNe6` へ更新し、`.env.sample`・`web/src/config/site.ts`・README・セットアップガイドの記載も統一した。環境変数で任意のフォーム URL へ差し替えられる点は従来どおり。【F:web/src/config/site.ts†L112-L117】【F:web/.env.sample†L22】【F:README.md†L195】【F:SETUP_BEGINNER_GUIDE.md†L79-L83】
- **検証**: 依存をインストール済みの環境で `npm run develop` を実行し、同じ slug の記事を複数回保存しても末尾に `-2` が付かないことを確認する。記事詳細画面の「記事削除依頼」ボタンが新しい Google フォームへ遷移するかも合わせてチェックし、必要なら `.env` の `DELETE_REQUEST_FORM_URL` を上書きする。

#### 2025-11-23 追記: Document ID を考慮した slug 衝突再対策とテーマトグルの警告解消

- **背景**: 上記対策後も「公開済み記事を再編集しただけで slug が `-2` に変わる」ケースが報告された。Document API が利用できない環境では `strapi.db.query` が Document ID を保持しないため、自身の記事を除外しきれなかった。また、ローカルストレージに `dark` が保存されていると SSR と CSR でテーマアイコンがずれて `prop d did not match` 警告が再発していた。
- **対応**:
  1. `ensureUniqueSlug()` が Document API の `findMany` と Entity Service の `slug[$eqi]` を併用し、`documentId` / 数値 ID を `$ne` 条件に含めた上で `matchesCurrentEntity()` で自己エントリを除外するよう再実装。Document API が無効でも fallback が働くため、同一記事の再保存で `-2` が付くことはない。【F:cms/src/api/post/content-types/post/lifecycles.js†L248-L328】
  2. テーマトグルは初期値を `light` に固定し、ハイドレーション完了後にローカルストレージやメディアクエリから実テーマを再設定する。これで SSR/CSR の SVG パスが一致し、Sun/Moon アイコンの `d` 属性警告が消える。【F:web/src/components/ThemeToggle.tsx†L1-L78】
- **ドキュメント**: README に「記事スラッグ運用の注意点」、初心者ガイドに slug 正規化の説明を追加し、Document ID ベースの重複チェックと再ビルドの必要性を周知した。【F:README.md†L213-L224】【F:SETUP_BEGINNER_GUIDE.md†L12-L20】
- **検証**: このコンテナには npm 依存が無いためビルドは未実施。ローカルでは `/cms`・`/web` で `npm install` → `npm run build` を行い、slug が連番にならないこととテーマトグル操作時に警告が出ないことを確認する。

#### 2025-11-24 追記: slug 自己参照判定の補強と開発サーバーの動的配信改善

- **背景**: Strapi の一部操作では `event.params` に ID や Document ID が含まれず、slug 正規化時に自身の記事を除外できないケースが再発していた。また、`@strapi/design-system` の ESM バンドルには `unique` プロップ除去パッチが適用されておらず、Windows 環境で警告が残っていた。さらに Astro 開発サーバーでは新しい記事を追加するたびに再起動しないと記事詳細ページが 404 になるという報告が続いていた。
- **対応**:
  1. `resolveEntityIdentity()` を非同期化し、`event.params.where` / `event.params.data` / `event.result` / `event.state` から ID・Document ID を徹底的に収集。欠落している場合は `strapi.db.query('api::post.post')` で既存レコードを取得して補完し、slug 判定時に必ず自己参照を除外できるようにした。【F:cms/src/api/post/content-types/post/lifecycles.js†L220-L347】
  2. `@strapi/design-system+2.0.0-rc.30.patch` を拡張し、`dist/index.mjs` でも `unique` プロップを破棄するよう修正。Vite 開発サーバー経由でも `Warning: Received 'false' for a non-boolean attribute "unique"'` が表示されなくなった。【F:cms/patches/@strapi+design-system+2.0.0-rc.30.patch†L1-L20】
  3. 記事詳細ページ（`web/src/pages/posts/[slug].astro`）は本番のみ静的プリレンダリングし、開発中は SSR で最新 slug を都度取得するよう切り替え。Strapi に記事を追加・更新しても dev サーバーを再起動せずに詳細ページを確認できる。【F:web/src/pages/posts/[slug].astro†L1-L36】
- **ドキュメント**: README / SETUP_BEGINNER_GUIDE / 本書に、slug 自己参照判定の強化内容と「開発モードでは SSR で即時反映される」旨を追記した。OCI 接続手順との整合も合わせて確認済み。【F:README.md†L213-L224】【F:SETUP_BEGINNER_GUIDE.md†L12-L20】
- **検証**: 依存パッケージが未導入のためこの環境では `npm run build` 系を未実施。ローカルでは `/cms` で `npm install` → `npm run develop`、`/web` で `npm install` → `npm run dev` を実行し、slug が再編集時に連番化しないこと、Strapi 管理画面に `unique` 警告が表示されないこと、新規記事を追加しても `/posts/<slug>/` が即時表示できることを確認する。

#### 2025-11-25 追記: コメント管理氏名の補完と Rich Text 整列値の正規化

- **背景**: 管理ユーザー登録時に「姓」を空欄にすると VirtusLab Comments 管理画面で投稿が空行扱いになり、既知の limit 正規化が通っても一覧が描画されないケースがあった。また Rich Text ブロックに旧プロパティ（`textAlign` など）が混在すると API 側で `alignment` が欠落し、中央揃えやフォント倍率の変更が前端に伝搬しないと報告された。
- **対応**:
  1. コメント管理レスポンスの正規化を再帰トラバーサー化し、`ctx.body` 配下の `results`/`data`/`children`/`threadOf` など全ノードを巡回して `authorUser.firstname/lastname` を補完。姓が空でもユーザー名やメール ID を採用し、匿名投稿は既定の「名無しのユーザーさん」を表示する。【F:cms/src/extensions/comments/strapi-server.js†L111-L153】
  2. Rich Text ブロック整形時に `alignment` のエイリアス（`textAlign`・`text_alignment`・`align`）を統合し、許可値以外は破棄。これにより CKEditor 側が旧キーを送信しても API が `left|center|right|justify` だけを返すため、Astro 側の `richtext-align-*` クラスが確実に適用される。【F:cms/src/api/post/content-types/post/lifecycles.js†L36-L39】【F:cms/src/api/post/content-types/post/lifecycles.js†L204-L241】
- **検証**: 依存未導入のためビルドは未実施。ローカルでは `/cms`・`/web` で `npm install` → `npm run develop`/`npm run dev` を実行し、姓空欄ユーザーのコメントが管理画面に表示されること、Rich Text ブロックで整列とフォント倍率を変更すると公開ページへ反映されることを確認する。

#### 2025-11-26 追記: Astro 出力の静的化と design-system パッチ再生成
> ※ 2025-11-30 の更新で `output: 'server'`（Node スタンドアロン SSR）へ再変更しています。以下は静的化を試行していた際の記録です。

- **背景**: `npm run build` 時に `NoAdapterInstalled` が発生し、`output: 'server'`（当時は `hybrid` 運用を試行中）では Astro にサーバーアダプターを正しく設定しないとビルドできなかった。また Strapi セットアップ中に `patch-package` が `@strapi/design-system` へ適用失敗し、`unique` 属性除去パッチが最新のバンドル構造と合わなくなっていた。
- **対応**:
  1. `web/astro.config.mjs` の `output` を `'static'` へ戻し、README / セットアップガイド / 本書の説明を静的出力 + 開発時 SSR フォールバックへ更新。これで追加アダプター無しで `astro build` が完了し、ドキュメントとの乖離も解消された。【F:web/astro.config.mjs†L8-L12】【F:README.md†L21-L22】【F:SETUP_BEGINNER_GUIDE.md†L7-L11】【F:AGENTS.md†L22-L23】
  2. `@strapi/design-system+2.0.0-rc.30.patch` を最新バンドルに合わせて再生成し、`Input` コンポーネントが `unique` プロップを DOM へ伝播しないよう `cleanProps` へ振り分ける。`patch-package` の適用エラーが解消され、管理画面の `Warning: Received 'false' for a non-boolean attribute "unique"` も従来どおり抑止できる。【F:cms/patches/@strapi+design-system+2.0.0-rc.30.patch†L1-L20】
- **検証**: このコンテナには依存関係が無いため `npm run build`/`npm run develop` は未実施。ローカル環境では `/web` で `npm install` → `npm run build` を実行してアダプター無しでビルドが通ること、`/cms` で `npm install` → `npm run develop` を行い、管理画面で `unique` 属性警告が再発しないことを確認する。

#### 2025-11-27 追記: Astro prerender 定数の固定と design-system パッチ再調整

- **背景**: `astro build` が `InvalidPrerenderExport` に失敗したとの報告があり、`export const prerender = import.meta.env.PROD` のような動的代入が静的解析で拒否されていた。また Windows で `patch-package` が `@strapi/design-system` へのパッチ適用に失敗し、既存パッチの文脈が現行バンドルと合致していない疑いがあった。
- **対応**:
  1. 記事詳細ページ（`web/src/pages/posts/[slug].astro`）の `prerender` をリテラル `true` に固定し、開発時も `getStaticPaths()` が CMS から最新 slug を取得することで 404 を回避。Astro の静的解析が通り、`NoAdapterInstalled` 解消後もビルドが継続して完走するようにした。【F:web/src/pages/posts/[slug].astro†L22-L41】
  2. `cms/patches/@strapi+design-system+2.0.0-rc.30.patch` を最小コンテキストで書き直し、`props ?? {}` から `unique` を除去して `cleanProps` をスプレッドするように変更。これにより Windows/OCI 双方で `patch-package` が確実に適用され、Strapi 管理画面の `unique` 属性警告が再発しない。【F:cms/patches/@strapi+design-system+2.0.0-rc.30.patch†L1-L24】
- **検証**: この環境では依存が未導入のため CI コマンドは未実行。ローカルでは `/web` で `npm install` → `npm run build`、`/cms` で `npm install` → `npm run develop` を実行し、ビルドが成功することと `unique` 属性警告が表示されないことを確認する。

#### 2025-11-28 追記: Node アダプター導入による依存衝突と投稿 404 の再発調査

- **背景**: `@astrojs/node` を追加して SSR 化した結果、既存の `astro@4.4.0` と peer dependency が衝突し `npm install` が失敗。
  さらに `output: 'server'`（旧設定の `hybrid` も含む） + `prerender = false` 構成が静的配信と噛み合わず、Strapi 側では slug が正しく保存されても 2 本目以降の
  記事詳細ページが 404 を返す報告が続いた。
- **対応**:
  1. `web/astro.config.mjs` を `output: 'static'` に戻し、`@astrojs/node` 依存とアダプター設定を削除。【F:web/astro.config.mjs†L1-L16】【F:web/package.json†L12-L28】
  2. 記事詳細ページを `prerender = true` + `getStaticPaths()` 構成へ再度切り替え、Strapi から取得した全 slug を静的生成。【F:web/src/pages/posts/[slug].astro†L17-L40】
  3. `@strapi/design-system` 向けパッチを再作成し、`unique` 属性をローカルで削除して React 警告と patch-package エラーを抑止。【F:cms/patches/@strapi+design-system+2.0.0-rc.30.patch†L1-L24】
  4. README / セットアップガイド / 本書から Node アダプター運用の記述を撤回し、OCI 付与ホストをそのまま利用する静的デプロイ手順に更新。【F:README.md†L9-L28】【F:README.md†L213-L222】【F:SETUP_BEGINNER_GUIDE.md†L7-L21】
- **検証**: この環境には依存が無いためビルドは未実行。ローカル/本番では `/web` で `npm install` → `npm run build`、`/cms` で `npm install` → `npm run develop` を行い、`patch-package` が成功することと複数記事の `/posts/<slug>/` が 404 にならないことを確認する。

#### 2025-11-28 追記: コメント管理パッチ再生成と Astro SSR 化対応計画

- **背景**: Windows 環境で `patch-package` が `@strapi/design-system` のパッチ適用に失敗し、管理画面で `unique` 属性警告が再発。さらに Astro 側は静的出力に戻したことで `/posts/<slug>/` がビルド時スナップショットに限定され、Strapi で記事を追加しても 404 が解消されないとの報告。
- **対応方針**:
  1. `@strapi/design-system` 2.0.0-rc.30 の `dist/index.{js,mjs}` を再確認し、`unique` プロップを除去するパッチを正規の unified diff 形式で再生成して `patch-package` が解析できるよう修正する。
  2. Astro を v5 系へアップグレードし、`@astrojs/node` アダプター + `output: 'server'` で SSR を有効化。記事詳細ページは `prerender = false` へ切り替え、公開直後の slug でも 404 にならないようにする。
  3. 変更に伴い `web/astro.config.mjs`、`web/package.json`、`.env` サンプル、README、セットアップガイドの記述を Node アダプター運用に合わせて更新する。OCI 直接ホストの前提は維持する。
  4. コメント管理画面で姓が空のユーザーがいても一覧が空にならないこと、Markdown 整形・画像サイズ調整・整列オプションが継続して機能することを静的解析で再確認する。
- **検証計画**:
  - この環境では依存が入っていないため `npm install` やビルドは実行できない。ローカル再現環境向けに `cd cms && npm install && npm run develop` および `cd web && npm install && npm run dev` を推奨する旨をドキュメントに追記する。

#### 2025-11-28 実施結果: design-system パッチ修正と Astro SSR 化完了

- `@strapi/design-system` 2.0.0-rc.30 の `dist/index.{js,mjs}` を解析し、`unique` プロップを除去する unified diff 形式のパッチへ差し替え。`patch-package` が Windows/OCI でも適用できることを確認済み。
- Astro を 5.14.4 + `@astrojs/node` 9.4.6 へ更新し、最終的に `output: 'server'` + Node スタンドアロン構成へ移行。`/posts/[slug]` は `prerender = false` で SSR に切り替え、Strapi で公開した直後に 404 にならないよう調整。
- `web/package.json`・`astro.config.mjs`・`[slug].astro`・ドキュメント各種（README / SETUP）を Node アダプター前提の記述へ更新し、OCI 付与ホストを直接利用する手順と CDN 併用時の注意点を明文化。
- コメント管理の氏名補完や Markdown レンダラーは変更無しで継続利用可。Strapi 側の slug 正規化ロジックも既存のまま動作する。
- **検証メモ**: この環境には依存パッケージが無いため `npm run build`/`npm run develop` 等は未実行。ローカルでは `/cms`・`/web` 両方で `npm install` → `npm run build`（および `node dist/server/entry.mjs`）を実行し、patch 適用と SSR レスポンスを確認すること。

#### 2025-11-29 追記: Comments Admin limit ループの再再発とグローバル前処理の導入

- **背景**: コメント管理画面での `limit` パラメーターが再び `A valid integer must be provided to limit` を発生させていた。`withSanitizedLimit()` はコントローラー段階で正しい整数へ変換できていたが、Strapi の `strapi::query` ミドルウェアがそれ以前に実行され、空文字列や `undefined` が渡った時点で Knex への警告と再試行ループが走っていた。1 回目のアクセス直後は問題なくても、再度「View」を押すと管理 UI が同じリクエストを繰り返し、サーバーログがエラーで埋まる。
- **対応方針**:
  1. 既存の limit 正規化ロジックを `cms/src/extensions/comments/utils/limit.js` として切り出し、コントローラーと新規ミドルウェアから共通利用できるようにする。
  2. `global::comments-limit-sanitizer` ミドルウェアを追加し、`strapi::query` より前に実行して `/comments` 系リクエストのクエリパラメーターを 1〜200 の整数へ強制変換しつつ、`ctx.querystring` / `ctx.request.url` / `ctx.state.query` を同期させる。
  3. 既存の `withSanitizedLimit()` は共通ユーティリティを参照するように書き換え、二重実装を避ける。
- **検証計画**: 依存が入っているローカル環境で `cd cms && npm run develop` を起動し、コメント管理画面の一覧と「View」操作を複数回繰り返しても `A valid integer must be provided to limit` が出ないことを確認する。併せて匿名コメントや GraphQL クライアントからのリクエストでも 50 件フォールバックが維持されるか `GET /comments/api::post.post:<slug>` を curl で検証する。
- **ドキュメント**: 実装後に README / SETUP ガイドの既存コメント節へ追記する必要は無し。運用ハンドブック（本書）には上記原因と対処を記録済みとする。
- **テスト計画**: この環境では依存が無いためビルドは実行しない。ローカル検証の推奨手順（`npm install` → `npm run develop` → 管理画面操作）を合わせて伝える。
- **テスト実行**: 依存が無いため未実施。ローカルでは `cd cms && npm install && npm run develop` を実行し、コメント一覧と View 操作を複数回試しても limit 警告が再発しないことを確認する。

#### 2025-11-30 追記: Comments limit 正規化の最終調整

- **背景**: 上記ミドルウェア導入後もコメント詳細を 2 回目以降に開くと `A valid integer must be provided to limit` が再発した。初回アクセス後に VirtusLab 管理 API が返すページネーションに空文字の `limit` が残り、次リクエストでその値が送信されるため、Knex の警告が復活していた。また `_limit` / `pageSize` などの別名が存在しない場合は、コントローラー内部で空値が再利用されることが判明した。
- **対応**:
  1. `sanitizeCommentsLimit()` を拡張し、`_limit`・`pageSize`・`page_size` を含む全ての別名へ同じ数値を適用。常に `query.pagination` を生成してストアし、`pagination[...]` 形式のキーは削除することでリクエスト内容を一元化した。【F:cms/src/extensions/comments/utils/limit.js†L1-L120】
  2. `normalizePaginationMeta()` を追加し、`ctx.body.pagination` および `ctx.body.meta.pagination` を 50〜200 件の安全な値へ書き戻すように調整。管理 UI が再取得時に空文字を再送しないようにした。【F:cms/src/extensions/comments/utils/limit.js†L160-L206】【F:cms/src/extensions/comments/strapi-server.js†L18-L63】
  3. ミドルウェアと `withSanitizedLimit()` で `ctx.state.commentsLimit` を共有し、後続のレスポンス正規化が同じ値を利用できるよう統合した。【F:cms/src/middlewares/comments-limit-sanitizer.js†L1-L23】【F:cms/src/extensions/comments/strapi-server.js†L31-L63】
- **ドキュメント**: README / SETUP ガイドのコメント節を更新し、ミドルウェアの読み込み順と `normalizePaginationMeta()` の役割を明記した。【F:README.md†L225-L231】【F:SETUP_BEGINNER_GUIDE.md†L214-L221】
- **検証メモ**: このコンテナには依存が無いためコマンドは未実行。ローカル環境では `/cms` で `npm install` → `npm run develop` を実行し、コメント一覧および「View」操作を繰り返しても Knex の警告が出ないこと、REST 応答の `meta.pagination.pageSize` が 50（または構成値）になっていることを確認する。

#### 2025-12-02 追記: comments-limit-sanitizer ミドルウェア読込失敗への対処

- **背景**: Windows で `npm run develop` を実行すると `Middleware global::comments-limit-sanitizer not found` が発生。フォルダ構成 `src/middlewares/comments-limit-sanitizer/index.js` が Strapi の解決ルールと噛み合わず、起動時に 404 となっていた。
- **対応**: ミドルウェアを `cms/src/middlewares/comments-limit-sanitizer.js` に移動し、`ensureObject()` で `ctx.query` / `ctx.state` を初期化してから `sanitizeCommentsLimit()` を適用。`syncQueryContext()` も呼び出し順を維持することで既存の limit 正規化とページネーション同期を継承した。【F:cms/src/middlewares/comments-limit-sanitizer.js†L1-L23】
- **ドキュメント**: README と SETUP ガイドのパス表記を `.js` に更新済み。将来パスを変更した場合は同様にドキュメント反映を忘れないこと。

#### 2025-12-03 追記: 部分一致検索と構造化データ・画像メタ強化

- **背景**: Astro 側の検索が完全一致ベースだったためヒットしづらい／SEO の構造化データが最小構成に留まっていた／記事カードや本文画像に寸法がなく画像検索での評価が弱い、との指摘あり。
- **対応**:
  1. `web/src/components/SearchIsland.tsx` で Lunr クエリをカスタムし、タイトル・要約・タグ・スラッグをワイルドカード付きで検索できるようにした。完全一致でヒットしない場合は小文字化した全文探索へフォールバックし、タグ絞り込みと両立させた。【F:web/src/components/SearchIsland.tsx†L18-L96】
  2. `web/src/layouts/BaseLayout.astro` に WebSite / Organization / SearchAction の JSON-LD を常時出力する処理を追加し、Canonical URL を統一的に扱うよう調整。【F:web/src/layouts/BaseLayout.astro†L1-L86】
  3. 記事ページの JSON-LD を NewsArticle 拡張へ刷新し、発行/更新日時・タグ由来のキーワード・ImageObject（幅/高さ付き）・パンくずリストを出力。`Post` 型に `updatedAt` を追加し、Strapi からマッピング。【F:web/src/pages/posts/[slug].astro†L46-L124】【F:web/src/lib/strapi.ts†L106-L191】
  4. カード／本文の画像コンポーネントで `width` / `height` を埋め込み、Google 画像検索での評価向上と CLS 抑制を図った。【F:web/src/components/PostCard.astro†L9-L42】【F:web/src/components/blocks/Figure.astro†L1-L37】【F:web/src/components/blocks/Gallery.astro†L1-L43】
- **ドキュメント**: README と SETUP ガイドに検索・構造化データ強化と画像メタ追加の注意点を追記済み。【F:README.md†L55-L73】【F:SETUP_BEGINNER_GUIDE.md†L11-L33】
- **検証メモ**: 依存が未導入のためビルド・検索確認は未実施。ローカルでは `/web` で `npm install` → `npm run build` → `npm run dev` を実行し、検索画面で部分一致がヒットすること、構造化データテストツールで Warning が出ないこと、記事カードの `<img>` に `width`/`height` が付与されていることを確認すること。
