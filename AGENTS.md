...
- `/web` を Astro Hybrid 構成へ切り替え、開発サーバーは SSR フォールバックで最新スラッグを都度取得可能にした。`astro.config.mjs` の `output` は `hybrid` を指定し、`/posts/[slug].astro` では本番時のみ静的プリレンダーを行う。
...
- `/web`: Astro 4.4 + React Islands。Cloudflare Pages へ静的デプロイする一方、開発時は SSR フォールバックで最新記事スラッグをオンデマンド取得できるハイブリッド構成。
