...
- **背景**: `npm run build` 時に `NoAdapterInstalled` が発生し、`output: 'hybrid'` のままでは Astro にサーバーアダプターが必須となるためビルドできなかった。また Strapi セットアップ中に `patch-package` が `@strapi/design-system` へ適用失敗し、`unique` 属性除去パッチが最新のバンドル構造と合わなくなっていた。
- **対応**:
...
- **背景**: `@astrojs/node` を追加して SSR 化した結果、既存の `astro@4.4.0` と peer dependency が衝突し `npm install` が失敗。
  さらに `output: 'hybrid'` + `prerender = false` 構成が静的配信と噛み合わず、Strapi 側では slug が正しく保存されても 2 本目以降の
...
- **対応方針**:
  ...
  2. Astro を v5 系へアップグレードし、`@astrojs/node` アダプター + `output: 'hybrid'` で SSR を有効化。記事詳細ページは `prerender = false` へ切り替え、公開直後の slug でも 404 にならないようにする。
...
- Astro を 5.14.4 + `@astrojs/node` 9.4.6 へ更新し、`output: 'hybrid'` + Node スタンドアロン構成へ移行。`/posts/[slug]` は `prerender = false` で SSR に切り替え、Strapi で公開した直後に 404 にならないよう調整。
