...
- **対応**:
  1. フロントエンドの Markdown 変換を `web/src/lib/strapi.ts` で全面刷新し、`marked` を使って CKEditor/Markdown の装飾（太字・斜体・取り消し線・インライン/ブロックコード・引用・リンク・画像・改行）を HTML へ変換するよう実装。画像・リンクは従来通り `ensureAbsoluteUrl` を通じて絶対 URL 化し、`<figure>` / `<a target="_blank">` を維持する。【F:web/src/lib/strapi.ts†L1-L236】
  2. 新しいレンダラーで利用する `marked` を `web/package.json` の依存関係へ追加し、Astro ビルド時に自動取得されるようにした。【F:web/package.json†L22-L34】
  3. README / SETUP ガイドを更新し、Rich Text ブロックが Markdown をそのまま再現できること・`marked` へ依存していることを明記して運用メモに追加。【F:README.md†L67-L74】【F:SETUP_BEGINNER_GUIDE.md†L204-L212】
- **検証**: このサンドボックスには Strapi/Astro の依存パッケージが導入されていないため自動テストは未実行。ローカルでは `cd web && npm install` を実行して `marked` を取得したうえで、Strapi の Rich Text フィールドに Markdown 装飾を含む記事を保存し、`npm run dev` / `npm run build` で装飾と改行が再現されることを確認すること。
- **今後の指針**: Markdown 変換に追加要件（表・脚注など）が出た場合は `marked` のプラグインやオプションで拡張する。HTML を直接保存するケースでは `normalizeRichMarkup()` が従来通り生の HTML を優先するため、不要なサニタイズやダブルエスケープが発生しないことを確認する。`marked` のセキュリティ更新が出た場合は速やかにバージョンを上げ、CHANGELOG を確認して互換性が保たれているかテストする。
...
- **対応**:
  1. `normalizeRichMarkup()` をエクスポートし、`RichTextContent` コンポーネント側でも毎回マークアップを再正規化するよう変更。API から HTML が届いた場合はそのまま描画し、Markdown/プレーンテキストだった場合はクライアント側でも `marked` で HTML 化する二重防御構成にした。【F:web/src/lib/strapi.ts†L200-L236】【F:web/src/components/blocks/RichTextContent.tsx†L1-L31】
...
- **今後の指針**: 追加の Markdown 方言に対応する場合は `normalizeRichMarkup()` を拡張しつつ、`RichTextContent` にも同様の更新を反映させて二重防御を維持する。管理メニューの構造が変わった際は UID/リンク重複の扱いを見直し、警告が再発した場合は Hook を更新するか公式 API への移行を検討する。
