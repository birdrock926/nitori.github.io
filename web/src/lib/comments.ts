...
const commentsPath = (documentId: string) => {
  const relation = `api::post.post:${documentId}`;
...
export const fetchComments = async (documentId: string, pageSize?: number): Promise<CommentNode[]> => {
  if (!documentId) {
    return [];
  }
  ...
  const fetchPage = async (page: number): Promise<CommentsPage> => {
    try {
      ...
      const response = await request(`${commentsPath(documentId)}?${searchParams.toString()}`);
...
export const submitComment = async (
  ...
  const data = await request(commentsPath(documentId), {
    method: 'POST',
