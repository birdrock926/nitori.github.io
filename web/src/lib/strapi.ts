import { COMMENT_PAGE_SIZE, STRAPI, TWITCH } from '@config/site';
...
export const getPostSlugs = async () => {
  const maxAttempts = import.meta.env.PROD ? 6 : 12;
  const intervalMs = import.meta.env.PROD ? 1500 : 2500;
...
export type ModerationReason =
  | { type: 'word'; matches: string[]; severity?: 'low' | 'medium' | 'high'; score?: number }
  | { type: 'pattern'; matches: string[]; severity?: 'low' | 'medium' | 'high'; score?: number }
  | { type: 'link-count'; count: number; severity?: 'low' | 'medium' | 'high'; score?: number }
  | { type: 'link-host'; hosts: string[]; severity?: 'low' | 'medium' | 'high'; score?: number };
...
export type CommentMeta = {
  ...
  moderationSeverity?: 'none' | 'low' | 'medium' | 'high';
};
...
export type CommentListResponse = {
  data: CommentNode[];
  meta?: { pagination?: CommentPagination };
};

export const fetchComments = async (postSlug: string, page = 1): Promise<CommentListResponse> => {
  const response = await fetchJSON<CommentListResponse>('/api/comments/list', {
    postSlug,
    page: String(page),
  });

  const normalizedPagination: CommentPagination = {
    page: response.meta?.pagination?.page ?? page,
    pageSize:
      response.meta?.pagination?.pageSize ??
      (response.data?.length ? Math.max(response.data.length, COMMENT_PAGE_SIZE) : COMMENT_PAGE_SIZE),
