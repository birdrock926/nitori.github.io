...
const mapPost = (apiPost: PostListResponse['data'][number]) => {
  const attr = (apiPost as any)?.attributes ?? (apiPost as any) ?? {};
  const coverSource = attr.cover ?? (apiPost as any)?.cover;
  const cover = normalizeMedia(parseMedia(coverSource?.data ?? coverSource));

  const rawTags = attr.tags ?? (apiPost as any)?.tags;
  const tagsArray = Array.isArray(rawTags?.data)
    ? rawTags.data
    : Array.isArray(rawTags)
    ? rawTags
    : [];

  const rawBlocks = attr.blocks ?? (apiPost as any)?.blocks;

  return {
    id: (apiPost as any)?.id ?? attr.id ?? 0,
    title: attr.title ?? '',
    slug: attr.slug ?? '',
    ...
    source: attr.source,
    cover,
    tags: tagsArray.map((tag: any) => ({ id: tag.id, ...(tag.attributes ?? tag) })),
    blocks: Array.isArray(rawBlocks) ? rawBlocks.map(normalizeBlock) : [],
