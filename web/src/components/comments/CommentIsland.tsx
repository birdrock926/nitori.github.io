...
import type { CommentNode, CommentPagination, ModerationReason } from '@lib/strapi';
...
import { CAPTCHA, COMMENT_PAGE_SIZE } from '@config/site';
...
const describeModerationReasons = (reasons: ModerationReason[] = []) => {
  ...
  const labels = reasons
    .map((reason) => {
      const severity = describeSeverity(reason.severity);
      if (reason.type === 'word') {
        return `${severity}特定語句（${reason.matches.join(', ')}）`;
      }
      if (reason.type === 'pattern') {
        return `${severity}特定表現（${reason.matches.join(', ')}）`;
      }
      if (reason.type === 'link-count') {
        return `${severity}リンク数が多い（${reason.count}件）`;
      }
      if (reason.type === 'link-host') {
        return `${severity}要確認リンク（${reason.hosts.join(', ')}）`;
      }
      return null;
    })
    .filter(Boolean);
...
const CommentIsland = ({ postSlug, defaultAlias }: Props) => {
  ...
  const defaultPagination: CommentPagination = {
    page: 1,
    pageCount: 1,
    pageSize: COMMENT_PAGE_SIZE,
    total: 0,
  };
  const [pagination, setPagination] = useState<CommentPagination>(defaultPagination);
  ...
  const loadComments = async (targetPage = 1) => {
    try {
      setLoading(true);
      const res = await fetchComments(postSlug, targetPage);
      const normalized = normalizeNodes(res.data ?? []);
      setComments(normalized);
      const meta = res.meta?.pagination ?? defaultPagination;
      setPagination(meta);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (comment.status !== 'published') {
        setComments((prev) =>
          parentId ? appendReply(prev, parentId, normalized) : [normalized, ...prev]
        );
      }
    } catch (err: any) {
      if (captchaProvider === 'turnstile' && window.turnstile && turnstileWidgetId.current) {
  ...
  const changePage = (target: number) => {
    if (target === currentPage || target < 1 || target > pagination.pageCount) {
      return;
    }
    loadComments(target);
  };
    ...
    <section aria-labelledby="comments-title" style={{ marginTop: '3rem' }}>
      ...
      <div style={{ marginTop: '2rem', display: 'grid', gap: '1.5rem' }}>
        ...
        {pagination.pageCount > 1 && (
          <nav className="comment-pagination" aria-label="コメントのページ切り替え">
            <button
              type="button"
              className="tag-chip"
            ...
            <ul className="comment-pagination__list">
              {pageNumbers.map((pageNumber, index) => {
                const previous = pageNumbers[index - 1];
                const needsEllipsis = previous && pageNumber - previous > 1;
                return (
                  <li key={pageNumber}>
                    {needsEllipsis && <span className="comment-pagination__ellipsis">…</span>}
                    <button
                      type="button"
                      className={pageNumber === currentPage ? 'tag-chip is-active' : 'tag-chip'}
