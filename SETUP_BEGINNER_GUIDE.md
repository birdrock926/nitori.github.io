...
| コメント即時公開 | `COMMENTS_AUTO_PUBLISH` | `smart`（既定）で健全な投稿のみ即時公開、`manual` ですべて承認待ち、`always` ですべて即時公開。 | `smart` |
...
- Strapi のサイドバー「Content Manager → Comments」を開くと、最新投稿が `status` 列付きで表示されます。カスタムレイアウトにより「対象記事 / 投稿者 / 状態 / 投稿日時」が常に並び、`状態` 列は色付きバッジで判別できます。初期状態は `pending` なので、内容を確認して問題なければ `published` に更新してください。旧環境からコピーした DB で列が不足していても、起動時のブートストラップ処理が `comments.status` / `comments.meta` を自動で追加し、未設定行を `pending` に揃えます。
- Windows のようにコロン (`:`) を含むパスを作成できない環境でも、ブートストラップ処理が Content Manager の設定を直接 `core_store` に書き込むため、一覧と編集画面に「対象記事」「状態」「投稿日」が必ず表示されます。
- 一覧には「対象記事」列が追加されているため、どの記事のコメントかをモデレーターが即座に判断できます。タイトルをクリックすると該当記事へジャンプします。
- コメントを公開する手順：対象コメントをクリック → 右側の `status` プルダウンで `published` を選択 → 右上の **保存** をクリック。保存後は 1〜2 秒で Web 側へ反映されます。
- Web 側のコメント欄は 20 件ごとにページングされており、「前へ」「次へ」ボタンで過去ログをたどれます。審査後に公開したコメントも 1 ページ目へ自動的に並ぶため、表示確認が容易です。
- `status` プルダウンが表示されない場合は、一度 CMS を再起動してください。起動時に Content Manager 用レイアウトが強制的に再適用され、`post / alias / status / createdAt` の一覧列と編集フォームが復旧します。
- 一覧行の右端「︙」メニューにも「公開」「非表示」「シャドウ」ボタンを追加しており、ワンクリックでステータス変更できます（内部的に `/api/mod/comments/:id/(publish|hide|shadow)` を呼び出しています）。
- 返信コメントを登録する際は編集画面の `parent` フィールドから親コメントを選択します。一覧の「対象記事」列をクリックすると記事編集画面へジャンプできるため、どの投稿に紐づくスレッドか即座に把握できます。
- コメント処理のコードは `/cms/src/modules/comments/` にモジュール単位でまとまっています。`moderation.js` は本文検証、`rate-limit.js` は投稿頻度チェック、`transform.js` は公開レスポンス整形を担当するため、必要に応じてロジックを差し替えてください。
...
- 本文が禁止語・未許可ドメイン・リンク過多のインデックスに該当すると自動的に `pending` となり、理由が `meta.moderation.reasons` に記録されます。既定の `COMMENTS_AUTO_PUBLISH=smart` では健全な投稿のみ即時公開され、全件を事前審査したいときだけ `manual` へ切り替えます。常時即時公開したい場合は `always` を利用します。
- コメント詳細画面右上の「︙」→ **JSON を表示** を開くと、`meta.client.ip` と `maskedIp`、`ua`、`submittedAt` に加えて `meta.display.postTitle` / `postSlug` も確認できます。API で確認したい場合は管理者トークンを付与して `GET /api/mod/comments/:id/meta` を呼び出してください。
- API の呼び出し例
  ```bash
