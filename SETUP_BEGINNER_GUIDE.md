...
- **Web (/web)**: Astro + React Islands で構成された静的サイト。`getStaticPaths()` が Strapi の公開スラッグを収集してビルドし、GitHub Actions 経由で Cloudflare Pages などへデプロイします。ローカル開発中に記事を追加した場合は `npm run dev` を再起動して最新スラッグを取得してください。
...
- **開発中の slug 反映**: `/posts/[slug].astro` は静的プリレンダーを利用しています。Strapi で新しい記事を公開したら、Astro 開発サーバーを一度停止して再起動すると最新スラッグが反映されます。本番では Strapi → GitHub Actions → Cloudflare Pages の自動再ビルドにより数分以内に公開されます。
