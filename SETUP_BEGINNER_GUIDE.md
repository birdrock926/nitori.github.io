...
- Rich Text ブロックの本文は CKEditor の Markdown 互換テキストを `marked` で HTML へ変換し、太字・斜体・取り消し線・コード（インライン/ブロック）・引用・リンク・画像・改行をそのまま再現します。Strapi 側で HTML が生成されなかった場合でも、公開ページでは装飾が崩れません。フロントエンドの描画時にも同じ正規化が再実行されるため、API レスポンスがプレーンテキストのまま渡ってきても公開画面では Markdown 記号が残りません。
