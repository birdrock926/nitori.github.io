...
7. コメントフォームのメール欄は任意入力ですが、VirtusLab Comments 3.1.0 がメールアドレスを必須項目として検証するため、空欄や不正な値で送信した場合はバックエンドが `@comments.local` ドメインのダミーアドレスを自動補完して投稿を受け付けます（ダミー宛に通知は送信されません）。返信通知を受け取りたい場合は正しいメールアドレスを入力してください（API から外部公開はされません）。
