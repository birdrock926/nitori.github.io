...
- **構造化データ**: `web/src/layouts/BaseLayout.astro` で WebSite/Organization/SearchAction スキーマを常時出力し、記事ページでは NewsArticle JSON-LD に発行日時・更新日時・タグ由来のキーワード・ImageObject（幅/高さ付き）を含めています。Google 構造化データテストで警告が出ないか、公開前に一度チェックしてください。
