...
- **カスタムフィールド**: Rich Text ブロック向けの Typography Scale（文字サイズ倍率）フィールドは、Strapi 5.26 が props を未初期化のまま渡すケースやオプションを配列で保持するケースでも安全に動作するよう `min/max/step/defaultScale` だけを抽出する防御ロジックを実装済み。Admin 拡張側でも `window.__plugin_typography_scale_field_registered__` / `__plugin_typography_scale_admin_registered__` による冪等ガードで登録を 1 回に固定し、`lazyLoadComponents → setStore` ループを封じています。未入力時は記事既定値 (1.0) を使い、0.7〜1.8 倍の範囲をスライダーと数値入力で調整できます。Intl コンテキストが読み込まれる前でも `window.strapi` 経由のフォールバックでラベルが表示されます。2025-10-16 の更新で実装を `React.PureComponent` ベースの完全ステートレス構成に置き換えたため、Strapi がフィールドを React Dispatcher 未初期化のまま直接評価しても純粋な React 要素を返すだけで処理が終了し、ローカル state の競合が原因となる `Invalid hook call`、画面フリーズ、無限リロードは再発しません。2025-10-17 には `attribute`/`attributeOptions` 全体をスプレッドする旧処理を廃止し、`options.min` など必要なキーをパス指定で辿る抽出ロジックと props 参照に基づくキャッシュを導入したため、大量再評価でも CPU が張り付かず Create an entry → Posts → Rich Text を開いた直後のフリーズが解消されています。入力値はすべて props とその場の正規化結果から導出され、Strapi のフォームストアが唯一のソースとして保持されるようになっています。2025-10-18 では探索ノード数を 64 件に制限し、参照対象プロパティを Design System のオプション階層に限定したため、Dynamic Zone が巨大でも初期化処理が一定時間内で完了します。`npm ls react` を実行して React の複数バージョン混在が無いことも確認済みです。2025-10-19 時点では探索処理を事前定義したパスリストと `options.min` などのエイリアス照合だけに限定し、Strapi Blocks がフィールドを短時間に再評価しても幅優先探索が暴走しないよう調整済みです。2025-10-20 以降は候補ソースを `props` と浅い階層に限定し、既知の `entries/base/advanced` など最大 24 件の配列だけを走査する簡素化を施したため、Dynamic Zone に多数のコンポーネントが含まれていても schema 全体を再帰的に読み直すことはありません。`CI=1 npm run build` とブラウザ検証で TypeError や描画ループが再発しないことを確認し、プラグインの `package.json` と `npm ls react` で React 依存が重複していないことも監査しています。2025-10-21 更新では Dynamic Zone 追加直後に `mainState` が未初期化の巨大オブジェクトを返してくるケースを再度検証し、継承ロジックを完全に停止。`min/max/step/defaultScale` は `options`・`attribute.options`・`attributeOptions` など浅い階層からのみ直接参照し、`base` や `entries` といった配列は読み取らずに未指定時は既定値へ即時フォールバックする構成に変更した。これにより schema 全体を辿る処理はなくなり、Create an entry → Posts → Rich Text を選択した直後でも処理時間が一定に保たれることをブラウザ検証と `CI=1 npm run build` で再確認済み。React バージョンの重複を避けるための `npm ls react` チェックも運用ルールとして継続する。2025-10-22 の更新では Dynamic Zone 初期化時に `value` が `undefined` のまま残ることで Strapi 側が再マウントを繰り返す問題に対応し、`ensureInitialValue()` が一度だけ `onChange({ value: null })` を送出して既定値を確定させるガードと、数値補正時に重複イベントを抑止するメモ化を追加しています。これによりブラウザで観測された `Warning: Maximum update depth exceeded` が解消され、Rich Text ブロックが即時に描画されることを再確認しました。 このフィールドの管理画面登録はグローバルシンボルで一度だけ実行されるようにしてあり、Strapi Admin が Dynamic Zone 初期化中に `lazyLoadComponents` から `setStore` を繰り返す無限更新が発生しないよう防いでいます。
