...
- Rich Text ブロックの本文は CKEditor の Markdown 互換テキストを `marked` ベースのレンダラーで HTML に変換し、太字・斜体・取り消し線・コード（インライン/ブロック）・引用・リンク・画像・リスト・改行をそのまま再現します。Strapi 側で HTML が生成されなかった場合でも公開ページでは装飾が崩れません。フロントエンドの描画時にも同じ正規化を再実行し、`<p>` や `<br>` だけでラップされた Markdown 文字列はいったんタグを除去してから再評価するため、保存内容が `**bold** _italic_` のまま表示されることはありません。2025-11-04 のアップデートで Markdown の再評価フローをサーバー/クライアント共通の決定的な手順に統一したため、ハイドレーション時の `Text content does not match server-rendered HTML` 警告や記号の生表示が再発してもリロードなしに解消されます。
