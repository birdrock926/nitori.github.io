...
- **カスタムフィールド**: Rich Text ブロック向けの Typography Scale（文字サイズ倍率）フィールドは、Strapi 5.26 が props を未初期化のまま渡すケースやオプションを配列で保持するケースでも安全に動作するよう防御ロジックを追加済み。未入力時は記事既定値 (1.0) を使い、0.7〜1.8 倍の範囲をスライダーと数値入力で調整できます。
