...
- Rich Text の本文は Markdown 記法や単純な改行を入力しても、Strapi のライフサイクルが `marked` レンダラーで自動的に HTML へ変換してから API へ返します。これにより Strapi のプレビューや公開サイトの SSR/CSR で記号そのものが露出することはありません。
