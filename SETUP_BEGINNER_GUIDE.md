...
- **カスタムフィールド**: 2025-10-27 に Font Scale 系プラグインを撤去し、Rich Text ブロックの `fontScale` は Strapi 標準の Decimal フィールドへ戻しました。Windows で `Unsupported field type: plugin::font-scale-range.scale` が解消しきれなかったためです。現在は管理画面の数値入力で 0.7〜1.8 の範囲を直接編集し、空欄なら記事既定値 (1.0 倍) を適用します。Strapi 5.26+ は `min/max/step` を `options.*` に置かないと管理画面初期化時に「'step' must be prefixed with 'options.'」エラーで停止するので、`cms/src/components/content/rich-text.json` を編集する際は必ず `options` 内に集約してください。加えて `alignment` 列挙（left/center/right/justify）で段落の整列方向を指定できるようにしました。値の丸めや上下限は `cms/src/api/post/content-types/post/lifecycles.js`（`clampScaleValue`）で制御しているので、必要に応じて調整してください。履歴と検証ログは AGENTS.md にまとまっています。
- Rich Text の本文は Markdown 記法や単純な改行を入力しても、Strapi のライフサイクルが `marked` レンダラーで自動的に HTML へ変換してから API へ返します。同じ設定を Astro 側でも `web/src/lib/richtext.ts` が利用しているため、SSR/CSR の差異によるハイドレーション警告や Markdown 記号の露出が抑制されます。
