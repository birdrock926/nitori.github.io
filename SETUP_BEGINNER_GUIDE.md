...
3. 主要項目のチェック
   ...
   | コメント即時公開 | `COMMENTS_AUTO_PUBLISH` | `smart`（既定）で健全な投稿のみ即時公開、`manual` で常に承認待ち、`always` ですべて即時公開。 | `smart` |
...
- 本文が禁止語・未許可ドメイン・リンク過多のインデックスに該当すると自動的に `pending` となり、理由が `meta.moderation.reasons` に記録されます。既定の `COMMENTS_AUTO_PUBLISH=smart` では、これらに触れない投稿が自動的に公開されます。
