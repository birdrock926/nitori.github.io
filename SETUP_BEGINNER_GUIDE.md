...
- Rich Text ブロックの本文は CKEditor の Markdown 互換テキストを `marked` ベースのレンダラーで HTML に変換し、太字・斜体・取り消し線・コード（インライン/ブロック）・引用・リンク・画像・リスト・改行をそのまま再現します。Strapi 側で HTML が生成されなかった場合でも公開ページやプレビューモードで装飾が崩れません。フロントエンドの描画時にも同じ正規化を再実行し、`<p>` や `<br>` だけでラップされた Markdown 文字列はいったんタグを除去してから再評価するため、保存内容が `**bold** _italic_` のまま表示されることはありません。2025-11-05 のアップデートで Markdown の再評価フローをサーバー/クライアント共通の「単純ラッパー除去 → `marked` レンダリング」に固定し、Strapi プレビューと本番表示の両方で改行欠落や `Text content does not match server-rendered HTML` 警告が再発しないことを確認しました。
