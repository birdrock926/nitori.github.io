...
- **Web (/web)**: Astro + React Islands を `@astrojs/node` アダプターで動かすハイブリッド SSR。トップや一覧はプリレンダーしつつ、`/posts/[slug]` は SSR で Strapi の最新スラッグを即時反映します。ローカルでは `npm run dev` が Strapi の起動を待機するため、サーバーを再起動せずに新着記事を確認できます。
