...
- **検索 UI**: `/search` ページは Lunr インデックスにタイトル・要約・タグ・スラッグを登録し、ワイルドカード付きクエリで部分一致をサポートします。タグフィルターとの併用でも高速に応答するので、複数キーワードで探す運用に向いています。
- **構造化データ**: `web/src/layouts/BaseLayout.astro` で WebSite/Organization/SearchAction スキーマを常時出力し、記事ページでは NewsArticle に加えて DiscussionForumPosting JSON-LD を配信します。本文サマリーと直近のコメント（最大 5 件 + 各返信）を自動抽出するので、Google 構造化データテストで NewsArticle/DiscussionForumPosting の両方に警告が出ないか公開前に確認してください。
